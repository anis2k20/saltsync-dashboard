...
<template>
  <aside
    :class="[
      'relative z-10 flex h-screen flex-col justify-between bg-secondary-light transition-all dark:bg-secondary-dark',
      isOpen ? 'w-60' : 'w-16',
    ]"
  >
    <img
    ...
    <div>
      <!--header-->
      <header class="relative z-20 flex h-[72px] items-center justify-between p-4">
        <img
          v-if="isDarkMode"
        ...
        />
        <img v-else src="@/assets/images/logo.svg" alt="logo" class="w-36 object-cover" />

        <button @click="toggleSidebar" class="text-fg-gray dark:text-text-primary-dark">
      ...
      <ul class="relative z-10 space-y-2 p-base">
        <SidebarItem v-for="item in sidebarItems" :key="item.title || item.path" :item="item" />
      </ul>
    </div>

    <!--footer-->
    <footer class="space-y-[10px] p-4">
      <div v-if="isOpen" class="rounded-lg bg-primary-light px-base py-xx3 dark:bg-white">
        <h3 class="font-secondary font-semibold text-text-primary-light dark:text-text-placeholder">
          Enterprise
        </h3>
